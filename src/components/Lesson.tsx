    ...
    <Link to={`/event/lesson/${slug}`} className="group">
      ...
      >
        <header className="flex items-center justify-between">
          ...
          <span className={classNames('text-xs rounded py-[0.125rem] px-2 text-white border  font-bold', { 'text-white': isActiveLesson, 'border-green-500': !isActiveLesson })}>
            {lessonType === 'live' ? 'AO VIVO' : 'AULA PRÁTICA'}
          </span>
        </header>

        <strong className={classNames('mt-5 block', { 'text-white': isActiveLesson, 'text-gray-200': !isActiveLesson })}>{title}</strong>
      </div>
    </Link>
  );
}
